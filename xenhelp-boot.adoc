...
CAUTION: You may need to adjust the kernel and/or bootloader to use the correct serial port to work with HVM. See xlink:xenhelp-basics.adoc#_serial_console_options[the overview on serial options] for details.
...
UEFI is the most persnickety of HVM boot options. The xl configuration is trivial, but actually can be tricky to set up beyond the configuration. SCSI devices can't be enabled (see xref:xenhelp-dis.adoc[Modern Disk Configuration in XL]). I have found that I have to enable graphics, watch the first boot via VNC, and ensure everything is working well. UEFI is provided via OVMF, and OVMF must be compiled with Xen support (see below)
...
IMPORTANT: Don't forget to adjust the kernel and bootloader to use the correct serial port to work with HVM. See xlink:xenhelp-basics.adoc#_serial_console_options[the overview on serial options] for details.
